...
from Driver_MyProfile import Ui_SignUp_Dialog as ProfileUI
from Driver_MyRoutes import Ui_SignUp_Dialog as RoutesUI
from Driver_MyLog import Ui_SignUp_Dialog as LogUI
from Driver_Notification import Ui_SignUp_Dialog as NotificationUI
from db_util import db_connection

import backend_driver_ui as backend
import datetime

driver_cur_id = "D857185609"
...
class ProfileWindow(QDialog):
    ...
    def load_data(self):
        profile = backend.get_driver_profile(driver_cur_id)
    ...
    def save_edit(self):
        ...
        backend.save_profile(updated_data, driver_cur_id)
...
class RoutesWindow(QDialog):
    ...
    def load_data(self):
        ...
        routes = backend.get_routes_by_date(today, driver_cur_id)
...
class LogWindow(QDialog):
    ...
    def load_data(self, filter_month=None):
        profile = backend.get_driver_profile(driver_cur_id)
        self.ui.label_2.setText(f"{profile['first_name']} {profile['last_name']}")
        logs = backend.get_trip_logs(driver_cur_id)
        table = self.ui.tableWidget
...
class NotificationWindow(QDialog):
    ...
    def load_data(self, filter_month=None):
        profile = backend.get_driver_profile(driver_cur_id)
        self.ui.label_2.setText(f"{profile['first_name']} {profile['last_name']}")
        messages = backend.get_notifications(driver_cur_id)
        if filter_month:
